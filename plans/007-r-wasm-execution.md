...
In the future there will be two modes of running pdit:

1. Locally, where you install pdit as an R package on your own machine, and start pdit from R with `pdit::start()`, which starts a web server. The frontend is served through the web server, and R code can be executed through an HTTP API.
2. WASM, where pdit is served as a static website, and R code is executed in the browser using WebAssembly.
...
```typescript
// 1. Parse code
await webR.evalRVoid(`.pdit_code <- ${JSON.stringify(code)}`);
await webR.evalRVoid(`
  .pdit_parsed <- parse(text = .pdit_code, keep.source = TRUE)
`);

// 2. Get number of expressions
const n = await webR.evalRNumber('length(.pdit_parsed)');
...
try {
  for (let i = 1; i <= n; i++) {
    ...
    const srcref = await shelter.evalR(`attr(.pdit_parsed[[${i}]], 'srcref')[[1]]`);
    ...
    const result = await shelter.captureR(`eval(.pdit_parsed[[${i}]])`);
