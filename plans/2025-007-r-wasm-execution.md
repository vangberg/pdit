...
In the future there will be two modes of running Rokko:

1. Locally, where you install Rokko as an R package on your own machine, and start Rokko from R with `rokko::start()`, which starts a web server. The frontend is served through the web server, and R code can be executed through an HTTP API.
2. WASM, where Rokko is served as a static website, and R code is executed in the browser using WebAssembly.
...
```typescript
// 1. Parse code
await webR.evalRVoid(`.rokko_code <- ${JSON.stringify(code)}`);
await webR.evalRVoid(`
  .rokko_parsed <- parse(text = .rokko_code, keep.source = TRUE)
`);

// 2. Get number of expressions
const n = await webR.evalRNumber('length(.rokko_parsed)');
...
try {
  for (let i = 1; i <= n; i++) {
    ...
    const srcref = await shelter.evalR(`attr(.rokko_parsed[[${i}]], 'srcref')[[1]]`);
