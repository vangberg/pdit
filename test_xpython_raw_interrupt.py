#!/usr/bin/env python3
"""Test interrupt with xpython-raw kernel."""

from jupyter_client import KernelManager
import time

def test_xpython_raw_interrupt():
    # Start xpython-raw kernel
    km = KernelManager(kernel_name='xpython-raw')
    km.start_kernel()
    kc = km.client()
    kc.start_channels()
    kc.wait_for_ready(timeout=30)

    print("Using xpython-raw kernel\n")
...
""")
    ...
    if got_result:
        print("\n✅ SUCCESS: xpython-raw kernel survived interrupt!")
    else:
        print("\n❌ FAILED: xpython-raw kernel didn't respond")
